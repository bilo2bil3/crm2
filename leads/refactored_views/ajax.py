...
@login_required
def change_status_selected_leads(request):
    if request.method == "POST":
        payload = json.loads(request.body)
        url = payload["url"]
        leads_to_change_status = list(map(int, payload["leads"]))
        qs = Lead.objects.filter(pk__in=leads_to_change_status)
        new_status = payload["new_status"]
        print(payload)
        print(new_status)
        new_status = Category.objects.get(name=new_status)
        qs.update(category=new_status)
        print(qs)
        return HttpResponseRedirect(url)